...
echo "GPU_TARGETS option passed after restricting xnack variants for gfx908: ${GPU_TARGETS_EXPANDED}"
...
cmake -GNinja ${CMAKE_ARGS} -DTENSILE_USE_OPENMP=ON -DGPU_TARGETS=${GPU_TARGETS_EXPANDED} -DAMDGPU_TARGETS=${GPU_TARGETS_EXPANDED} -DCMAKE_C_COMPILER="$BUILD_PREFIX/bin/amdclang" -DCMAKE_CXX_COMPILER="$BUILD_PREFIX/bin/amdclang++" -Bbuild -S projects/rocblas
cmake --build ./build
cmake --install ./build
